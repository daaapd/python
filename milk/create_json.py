...
def report_create_json(date_report):
    import sqlalchemy
    ...
    for i in range(0, (point_count//28)+1):
        for p in point[:28]:
            list_prod = []
            for i in mysql_list:
                if i['pointId']==p:
        ...
        #удаляем элемент который уже использован
        point.remove(p)
        import json
        import re
        ...
        #r = requests.post(url, data=json.dumps(d, ensure_ascii=False), headers=headers)
